            ...
            <div class="border-b border-gray-200 pb-6">
                ...
                <div class="space-y-4">
                    <div class="flex items-center justify-between">
                        <p class="text-sm text-gray-600">Gérez les images de cette race</p>
...
<script>
    ...
    function loadRace() {
        ...
        Object.keys(customFields).forEach(key => {
            ...
            if (element && currentRace[key]) {
                if (element.type === 'checkbox') {
                    element.checked = currentRace[key] === '1' || currentRace[key] === true;
                } else {
                    element.value = currentRace[key];
                }
            }
        });
    ...
    function loadExistingImages() {
        if (currentRace.images && Array.isArray(currentRace.images)) {
            currentRace.images.forEach((image, index) => {
                addImageField(image);
            });
        }
    ...
    function addImageField(existingImage = null) {
        ...
        imageField.innerHTML = `
            ...
            <div class="space-y-3">
                <div>
                    ...
                    <input type="url" id="${imageId}_url" name="images[${imageCounter}][url]" value="${existingImage ? existingImage.url : ''}" class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500 focus:border-green-500" placeholder="https://exemple.com/image.jpg">
                </div>
                
                <div>
                    <label for="${imageId}_title" class="block text-sm font-medium text-gray-700 mb-1">Titre (optionnel)</label>
                    <input type="text" id="${imageId}_title" name="images[${imageCounter}][title]" value="${existingImage ? existingImage.title || '' : ''}" class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500 focus:border-green-500" placeholder="Titre de l'image">
                </div>
                
                <div>
                    <label for="${imageId}_description" class="block text-sm font-medium text-gray-700 mb-1">Description (optionnel)</label>
                    <textarea id="${imageId}_description" name="images[${imageCounter}][description]" rows="2" class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500 focus:border-green-500" placeholder="Description de l'image">${existingImage ? existingImage.description || '' : ''}</textarea>
                </div>
        ...
        if (existingImage && existingImage.url) {
            updateImagePreview(imageId, existingImage.url);
        }
    }
