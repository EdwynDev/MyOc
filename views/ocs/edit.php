...
<script>
    ...
    function loadOC() {
        ...
        const data = JSON.parse(localStorage.getItem('oc_data') || '{}');
        currentOC = (data.ocs || []).find(oc => oc.id === ocId);
    ...
    function setupForm() {
        ...
        form.addEventListener('submit', function(e) {
            ...
            const ocId = document.getElementById('oc-id').value;
            const data = JSON.parse(localStorage.getItem('oc_data') || '{}');
            const ocIndex = (data.ocs || []).findIndex(oc => oc.id === ocId);
            
            if (ocIndex !== -1) {
                data.ocs[ocIndex] = { ...data.ocs[ocIndex], ...updates };
                localStorage.setItem('oc_data', JSON.stringify(data));
                
                alert('OC mis à jour avec succès !');
                window.location.href = '/ocs';
            } else {
                alert('Erreur lors de la mise à jour de l\'OC');
            }
        });
    }
    ...
    function deleteOC() {
        if (confirm('Êtes-vous sûr de vouloir supprimer cet OC ? Cette action est irréversible.')) {
            ...
            data.ocs = (data.ocs || []).filter(oc => oc.id !== ocId);
            localStorage.setItem('oc_data', JSON.stringify(data));
    ...
    function addImageInputWithData(image) {
        ...
        newImageInput.innerHTML = `
            <div>
                <label class="block text-sm font-medium text-gray-700 mb-2">Titre de l'image (optionnel)</label>
                <input type="text" class="image-title w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500" placeholder="ex: Portrait, Tenue de combat..." value="${image.title || ''}">
            </div>
            <div>
                <label class="block text-sm font-medium text-gray-700 mb-2">URL de l'image</label>
                <input type="url" class="image-url w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500" placeholder="https://exemple.com/image.jpg" value="${image.data || ''}">
            </div>
            <div>
                <label class="block text-sm font-medium text-gray-700 mb-2">Description (optionnel)</label>
                <textarea class="image-description w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500" rows="2" placeholder="Description de l'image...">${image.description || ''}</textarea>
            </div>
            <button type="button" onclick="removeImageInput(this)" class="text-red-600 hover:text-red-800 text-sm">
                Supprimer cette image
            </button>
